...
Imports DAL ''Added 7/16/2019 thomas downes 
...
Public Class FormLogin
    ...
    ''4/17/2018 td''Private _objSaveSettings As ClassSaveSettings ''Added 4/17/2018 Thomas DOWNES   
    Private _objSaveSettings As ClassSaveSettings_BySuffix ''Added 4/17/2018 Thomas DOWNES   
    ...
    Private Sub buttonOK_Click(sender As Object, e As EventArgs) Handles buttonOK.Click
        ...
        Dim boolLeaveWindowOpenIfFails As Boolean ''Added 2/21/2018 Thomas D.
        Dim boolLoginCredentialIsValid As Boolean ''Added 2/26/2018 Thomas D.
        Dim intPermssionsLevel As Integer ''Added 2/26/2018 Thomas D.
        ...
        boolLoginCredentialIsValid =
            CheckLoginCredential_Valid(intPermssionsLevel, boolLeaveWindowOpenIfFails)

        ''2/26/2018 TD''If (CheckLoginCredential_Valid(boolLeaveWindowOpenIfFails)) Then ''Conditioned 2/7/2018 Thomas DOWNES 
        If (boolLoginCredentialIsValid) Then ''Conditioned 2/7/2018 Thomas DOWNES 
            ''
            ''Great!  Run the application! 
            ''
            ''2/28/2018 TD''Dim objFormDataManager As New frmDataManager
            Dim objFormRecons As New FormReconcilePics

            ''Added 3/27/2018 Thomas DOWNES 
            objFormRecons.AdvancedMode = _boolTechSupport

            ''2/26/2018 TD''[objFormDataManager.LoginByTechSupport = True ''Added 2/22/2018 Thomas D. 
            ''2/28/2018 TD''objFormDataManager.LoginByTechSupport = _boolTechSupport ''2/26 TD''True ''Added 2/22/2018 Thomas D. 
            ''2/28/2018 TD''objFormDataManager.AdvancedMode = chkExportAdvanced.Checked ''Added 2/26/2018 Thomas D. 
            objFormRecons.ShowDialog()
            Me.Close()
    ...
    Private Sub imgCIS_Click(sender As Object, e As EventArgs) Handles imgCIS.Click
        ...
        ''Added 2/7/2018 Thomas DOWNES
        System.Diagnostics.Process.Start("http:\\www.cisolutions.com")
    ...
    Private Function CheckLoginCredential_CheckMDBs(par_objMSAccess As clsOLEDB_MSAccess, par_strLoginID As String,
        ...
        ElseIf (_boolTechSupport) Then ''Added 2/21/2018 Thomas D. 
            ...
            MsgBox("The following credential-data MDB was not found. (CI Verify)  " & vbNewLine & vbNewLine &
                    par_objMSAccess.MDBPathAndFile & vbNewLine & vbNewLine &
                   "Sorry!", vbExclamation)
            Return False ''Added 3/27/2018 Thomas D.  
        ...
        Const c_PromptTechOrAdminsToEnterPathToMDB As Boolean = True ''False ''False per Sammi F. & Jayce P. on 2/26. ---2/26/2018 Thomas D. 

        If (c_PromptTechOrAdminsToEnterPathToMDB) Then ''Condition added 2/26/2018 TD

            ''3/1/2018 TD''If (_boolTechSupport Or (par_strLoginID.ToUpper() = "Admin".ToUpper())) Then
            If (_boolTechSupport) Then
        ...
        Return False ''Login is unconfirmed. 
    ...
    Private Sub FormLogin_Load(sender As Object, e As EventArgs) Handles Me.Load
        ...
        ''Added 4/17/2018 Thomas DOWNES 
        '' 4-17-2018 td''_objSaveSettings = New ClassSaveSettings
        _objSaveSettings = New ClassSaveSettings_BySuffix

        ''4/17/2018 td''_objSaveSettings.Load_FromXMLFile()
        _objSaveSettings.Load_FromXMLFile("_Login")

        ''Added 4/17/2018 Thomas DOWNES 
        ''
        ''5/1/2018 TD''Me.Left = CInt(_objSaveSettings.WindowLeft)
        ''5/1/2018 TD''Me.Top = CInt(_objSaveSettings.WindowTop)

        With _objSaveSettings ''5/1/2018 TD
            ''5/1/2018 TD
            If (.WindowLeft > 0) Then Me.Left = CInt(.WindowLeft)
            If (.WindowTop > 0) Then Me.Top = CInt(.WindowTop)
        End With ''End of "With _objSaveSettings"

        ''Added 5/01/2018 Thomas DOWNES 
        ''5/1/2018 td''If (Me.Left < 0) Then Me.Left = 0
        ''5/1/2018 td''If (Me.Top < 0) Then Me.Top = 0

        If (Me.Left < 0) Then
            Me.Left = CInt(0.5 * (Screen.PrimaryScreen.Bounds.Width - Me.Width))
        End If ''End of "If (Me.Left < 0) Then"

        If (Me.Top < 0) Then
            Me.Top = CInt(0.5 * (Screen.PrimaryScreen.Bounds.Height - Me.Height))
        End If ''end of ""If (Me.Top < 0) Then"
    ...
    Private Sub FormLogin_Closing(sender As Object, e As CancelEventArgs) Handles Me.Closing
        ...
        ''Added 4/17/2018 Thomas DOWNES 
        _objSaveSettings.WindowLeft = Me.Left
        _objSaveSettings.WindowTop = Me.Top

        ''Added 4/17/2018 Thomas DOWNES 
        ''4/17/2018 td''_objSaveSettings.Save_ToXMLFile()
        _objSaveSettings.Save_ToXMLFile("_Login")
